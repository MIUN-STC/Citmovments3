...
void Thresh_Callback (int, void *)
{
	...
	cv::Canny (M2, Canny_Out, 100, 100 * 2, 3);
	//cv::findContours (Canny_Out, Contours, Hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, cv::Point (0, 0));
	cv::findContours (Canny_Out, Contours, Hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_TC89_L1, cv::Point (0, 0));
	
	for (size_t I = 0; I < Contours.size (); I = I + 1)
	{
		cv::drawContours (M3, Contours, I, cv::Scalar (255, 50, 100), 1, 8, Hierarchy, 0, cv::Point ());
	}
...
int main (int argc, char * argv [])
{
	...
	while (Should_Run)
	{
		...
		cv::GaussianBlur (M2, M2, cv::Size (3, 3), 3.5, 3.5);
